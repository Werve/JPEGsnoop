...
CString CDecodePs::DecodeIptcValue(teIptcType eIptcType, unsigned nFldCnt, unsigned long nPos) const
{
    ...
    case IPTC_T_NUM2:
        {
            unsigned nVal = m_pWBuf->BufX(nPos, nFldCnt, PS_BSWAP);
    ...
    case IPTC_T_STR:
        ...
        for (nInd = 0; nInd < nFldCnt; nInd++)
        {
            strByte.Format(_T("%c"), static_cast<TCHAR>(Buf(nPos++)));
            strVal += strByte;
        }
        strVal += _T("\"");
        break;
...
CString CDecodePs::PhotoshopParseGetLStrAsc(unsigned long& nPos) const
{
    ...
    {
        // 4 byte string
        strVal.Format(_T("%c%c%c%c"), static_cast<TCHAR>(Buf(nPos + 0)), static_cast<TCHAR>(Buf(nPos + 1)),
            static_cast<TCHAR>(Buf(nPos + 2)), static_cast<TCHAR>(Buf(nPos + 3)));
        nPos += 4;
    }
    return strVal;
}
...
void CDecodePs::PhotoshopParsePixelAspectRatio(unsigned long& nPos, unsigned nIndent)
{
    unsigned nVal = m_pWBuf->BufRdAdv4(nPos,PS_BSWAP);
    PhotoshopParseReportFldNum(nIndent,_T("Version"), nVal,_T(""));
    unsigned nVal1 = m_pWBuf->BufRdAdv4(nPos,PS_BSWAP);
    unsigned nVal2 = m_pWBuf->BufRdAdv4(nPos,PS_BSWAP);
...
{
    unsigned nVal = m_pWBuf->BufRdAdv4(nPos,PS_BSWAP);
    unsigned nUnit = m_pWBuf->BufRdAdv2(nPos,PS_BSWAP);
    CString strUnit = PhotoshopParseLookupEnum(BIM_T_ENUM_RESOLUTION_INFO_RES_UNIT, nUnit);
...
bool CDecodePs::PhotoshopParseLayerInfo(unsigned long& nPos, unsigned nIndent, CDIB* pDibTemp)
{
    ...
    for (unsigned nLayerInd = 0; (bDecOk) && (nLayerInd < nLayerCount); nLayerInd++)
    {
        ...
        unsigned nWidth = sLayerAllInfo.psLayers[nLayerInd].nWidth;
        unsigned nHeight = sLayerAllInfo.psLayers[nLayerInd].nHeight;
    ...
    signed nPad = nPosStart + nLayerLen - nPos;
    if (nPad > 0)
    {
        nPos += nPad;
    }
...
bool CDecodePs::PhotoshopParseLayerRecord(unsigned long& nPos, unsigned nIndent, tsLayerInfo* pLayerInfo)
{
    ...
    unsigned nRect1 = m_pWBuf->BufRdAdv4(nPos,PS_BSWAP);
    unsigned nRect2 = m_pWBuf->BufRdAdv4(nPos,PS_BSWAP);
    unsigned nRect3 = m_pWBuf->BufRdAdv4(nPos,PS_BSWAP);
    unsigned nRect4 = m_pWBuf->BufRdAdv4(nPos,PS_BSWAP);
    ...
    CString strBlendModeSig = m_pWBuf->BufReadStrn(nPos, 4);
    nPos += 4;
    ...
    if (bDecOk)
    {
        ...
        m_pWBuf->BufReadStrn(nPos, nLayerNameLen);
        nPos += nLayerNameLen;
...
bool CDecodePs::PhotoshopDecodeRowUncomp(unsigned long& nPos, unsigned nWidth, unsigned nHeight, unsigned nRow, unsigned nChanID, unsigned char* pDibBits) const
{
    ...
    {
        unsigned char nVal = m_pWBuf->BufRdAdv1(nPos,PS_BSWAP);

#ifdef PS_IMG_DEC_EN
        if (pDibBits)
        {
            unsigned nRowActual = nHeight - nRow - 1; // Need to flip vertical for DIB
            unsigned nPixByte = (nRowActual * nWidth + nCol) * sizeof(RGBQUAD);
...
bool CDecodePs::PhotoshopDecodeRowRle(unsigned long& nPos, unsigned nWidth, unsigned nHeight, unsigned nRow, unsigned nRowLen, unsigned nChanID, unsigned char* pDibBits) const
{
    ...
    unsigned nRowOffsetComp = 0;
    unsigned nRowOffsetDecomp = 0;
...
bool CDecodePs::PhotoshopParseAddtlLayerInfo(unsigned long& nPos, unsigned nIndent)
{
    ...
    CString strKey = m_pWBuf->BufReadStrn(nPos, 4);
    nPos += 4;
    ...
    */
    ...
    if (strKey == _T("luni"))
    {
        CString sStr = PhotoshopParseGetBimLStrUni(nPos, nPosOffset);
...
bool CDecodePs::PhotoshopParseImageResourceBlock(unsigned long& nPos, unsigned nIndent)
{
    ...
    m_pWBuf->BufReadStrn(nPos, nResNameLen);
    nPos += nResNameLen;
...
void CDecodePs::PhotoshopParseJpegQuality(unsigned long& nPos, unsigned nIndent, unsigned long /*nPosEnd*/)
{
    ...
    case 0xFFFD: m_nQualitySaveAs = 1;
        break;
    case 0xFFFE: m_nQualitySaveAs = 2;
        break;
    case 0xFFFF: m_nQualitySaveAs = 3;
        break;
    case 0x0000: m_nQualitySaveAs = 4;
        break;
    case 0x0001: m_nQualitySaveAs = 5;
        break;
    case 0x0002: m_nQualitySaveAs = 6;
        break;
    case 0x0003: m_nQualitySaveAs = 7;
        break;
    case 0x0004: m_nQualitySaveAs = 8;
        break;
    case 0x0005: m_nQualitySaveAs = 9;
        break;
    case 0x0006: m_nQualitySaveAs = 10;
        break;
    case 0x0007: m_nQualitySaveAs = 11;
        break;
    case 0x0008: m_nQualitySaveAs = 12;
        break;
    default: m_nQualitySaveAs = 0;
        break;
    }
    ...
    case 0x0000: strVal = _T("Standard");
        break;
    case 0x0001: strVal = _T("Optimized");
        break;
    case 0x0101: strVal = _T("Progressive");
        break;
    default: strVal = _T("???");
        break;
    }
    ...
    switch (nVal)
    {
    case 0x0001: strVal = _T("3 Scans");
        break;
    case 0x0002: strVal = _T("4 Scans");
        break;
    case 0x0003: strVal = _T("5 Scans");
        break;
    default: strVal = _T("???");
        break;
    }
...
void CDecodePs::PhotoshopParseDescriptor(unsigned long& nPos, unsigned nIndent)
{
    ...
    for (unsigned nDescInd = 0; nDescInd < nDescNumItems; nDescInd++)
    {
        ...
        CString strOsType;
        strOsType.Format(_T("%c%c%c%c"), static_cast<TCHAR>(Buf(nPos + 0)), static_cast<TCHAR>(Buf(nPos + 1)),
            static_cast<TCHAR>(Buf(nPos + 2)), static_cast<TCHAR>(Buf(nPos + 3)));
        nPos += 4;
...
{
    CString strVal = PhotoshopParseGetLStrAsc(nPos);
