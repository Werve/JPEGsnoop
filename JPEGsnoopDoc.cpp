...
BEGIN_MESSAGE_MAP(CJPEGsnoopDoc, CRichEditDoc)
    ...
    ON_COMMAND(ID_FILE_SAVE_AS, &CJPEGsnoopDoc::OnFileSaveAs)
    ON_COMMAND(ID_FILE_OFFSET, &CJPEGsnoopDoc::OnFileOffset)
    ON_COMMAND(ID_FILE_REPROCESS, &CJPEGsnoopDoc::OnFileReprocess)
    ON_COMMAND(ID_TOOLS_ADDCAMERATODB, &CJPEGsnoopDoc::OnToolsAddcameratodb)
    ON_COMMAND(ID_TOOLS_SEARCHFORWARD, &CJPEGsnoopDoc::OnToolsSearchforward)
    ON_COMMAND(ID_TOOLS_SEARCHREVERSE, &CJPEGsnoopDoc::OnToolsSearchreverse)
    ON_UPDATE_COMMAND_UI(ID_TOOLS_ADDCAMERATODB, &CJPEGsnoopDoc::OnUpdateToolsAddcameratodb)
    ON_UPDATE_COMMAND_UI(ID_TOOLS_SEARCHFORWARD, &CJPEGsnoopDoc::OnUpdateToolsSearchforward)
    ON_UPDATE_COMMAND_UI(ID_TOOLS_SEARCHREVERSE, &CJPEGsnoopDoc::OnUpdateToolsSearchreverse)
    ON_COMMAND_RANGE(ID_PREVIEW_RGB, ID_PREVIEW_CR, &CJPEGsnoopDoc::OnPreviewRng)
    ON_UPDATE_COMMAND_UI_RANGE(ID_PREVIEW_RGB, ID_PREVIEW_CR, &CJPEGsnoopDoc::OnUpdatePreviewRng)
    ON_COMMAND_RANGE(ID_IMAGEZOOM_ZOOMIN, ID_IMAGEZOOM_800, &CJPEGsnoopDoc::OnZoomRng)
    ON_UPDATE_COMMAND_UI_RANGE(ID_IMAGEZOOM_ZOOMIN, ID_IMAGEZOOM_800, &CJPEGsnoopDoc::OnUpdateZoomRng)
    ON_COMMAND(ID_TOOLS_SEARCHEXECUTABLEFORDQT, &CJPEGsnoopDoc::OnToolsSearchexecutablefordqt)
    ON_UPDATE_COMMAND_UI(ID_FILE_REPROCESS, &CJPEGsnoopDoc::OnUpdateFileReprocess)
    ON_UPDATE_COMMAND_UI(ID_FILE_SAVE_AS, &CJPEGsnoopDoc::OnUpdateFileSaveAs)
    ON_COMMAND(ID_TOOLS_EXTRACTEMBEDDEDJPEG, &CJPEGsnoopDoc::OnToolsExtractembeddedjpeg)
    ON_UPDATE_COMMAND_UI(ID_TOOLS_EXTRACTEMBEDDEDJPEG, &CJPEGsnoopDoc::OnUpdateToolsExtractembeddedjpeg)
    ON_COMMAND(ID_TOOLS_FILEOVERLAY, &CJPEGsnoopDoc::OnToolsFileoverlay)
    ON_UPDATE_COMMAND_UI(ID_TOOLS_FILEOVERLAY, &CJPEGsnoopDoc::OnUpdateToolsFileoverlay)
    ON_COMMAND(ID_TOOLS_LOOKUPMCUOFFSET, &CJPEGsnoopDoc::OnToolsLookupmcuoffset)
    ON_UPDATE_COMMAND_UI(ID_TOOLS_LOOKUPMCUOFFSET, &CJPEGsnoopDoc::OnUpdateToolsLookupmcuoffset)
    ON_COMMAND(ID_OVERLAYS_MCUGRID, &CJPEGsnoopDoc::OnOverlaysMcugrid)
    ON_UPDATE_COMMAND_UI(ID_OVERLAYS_MCUGRID, &CJPEGsnoopDoc::OnUpdateOverlaysMcugrid)
    ON_UPDATE_COMMAND_UI(ID_INDICATOR_YCC, &CJPEGsnoopDoc::OnUpdateIndicatorYcc)
    ON_UPDATE_COMMAND_UI(ID_INDICATOR_MCU, &CJPEGsnoopDoc::OnUpdateIndicatorMcu)
    ON_UPDATE_COMMAND_UI(ID_INDICATOR_FILEPOS, &CJPEGsnoopDoc::OnUpdateIndicatorFilePos)
    ON_COMMAND(ID_SCANSEGMENT_DETAILEDDECODE, &CJPEGsnoopDoc::OnScansegmentDetaileddecode)
    ON_UPDATE_COMMAND_UI(ID_SCANSEGMENT_DETAILEDDECODE, &CJPEGsnoopDoc::OnUpdateScansegmentDetaileddecode)
    ON_COMMAND(ID_TOOLS_EXPORTTIFF, &CJPEGsnoopDoc::OnToolsExporttiff)
    ON_UPDATE_COMMAND_UI(ID_TOOLS_EXPORTTIFF, &CJPEGsnoopDoc::OnUpdateToolsExporttiff)
END_MESSAGE_MAP()
